...
resource "aws_security_group_rule" "frontend_egress_all" {
  ...
  cidr_blocks       = ["0.0.0.0/0"]
  security_group_id = aws_security_group.frontend_service_sg.id
  description       = "Allow frontend tasks to reach outbound internet"
}
...
resource "aws_security_group_rule" "backend_egress_all" {
  ...
  cidr_blocks       = ["0.0.0.0/0"]
  security_group_id = aws_security_group.backend_service_sg.id
  description       = "Allow backend tasks to reach outbound internet"
}
...
resource "aws_lb_target_group" "go_demo_frontend_tg" {
  ...
  health_check {
    ...
    interval            = var.frontend_health_interval_seconds
    healthy_threshold   = 2
...
resource "aws_lb_target_group" "go_demo_backend_tg" {
  ...
  health_check {
    ...
    interval            = var.backend_health_interval_seconds
    healthy_threshold   = 2
    unhealthy_threshold = 3
    protocol            = "HTTP"
    matcher             = "200-399"
...
resource "aws_lb_listener_rule" "listener_rule_backend_api" {
...
resource "aws_ecs_service" "go_demo_frontend_service" {
...
resource "aws_ecs_service" "go_demo_backend_service" {
