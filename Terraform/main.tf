...
resource "aws_subnet" "go_demo_public_subnet_1" {
  vpc_id                  = aws_vpc.go_demo_vpc.id
...
resource "aws_subnet" "go_demo_public_subnet_2" {
  vpc_id                  = aws_vpc.go_demo_vpc.id
...
resource "aws_route_table" "go_demo_public_rt" {
  vpc_id = aws_vpc.go_demo_vpc.id
...
resource "aws_route_table_association" "go_demo_public_rta_1" {
  subnet_id      = aws_subnet.go_demo_public_subnet_1.id
  route_table_id = aws_route_table.go_demo_public_rt.id
}

resource "aws_route_table_association" "go_demo_public_rta_2" {
  subnet_id      = aws_subnet.go_demo_public_subnet_2.id
  route_table_id = aws_route_table.go_demo_public_rt.id
}
...
resource "aws_security_group_rule" "alb_egress_to_frontend" {
  ...
  cidr_blocks       = ["0.0.0.0/0"]
  security_group_id = aws_security_group.alb_sg.id
  description       = "ALB egress to frontend/backends (open)"
}
...
resource "aws_security_group_rule" "alb_egress_to_backend" {
  ...
  cidr_blocks       = ["0.0.0.0/0"]
  security_group_id = aws_security_group.alb_sg.id
  description       = "ALB egress to backend (open)"
}
...
resource "aws_security_group_rule" "frontend_ingress_from_alb_80" {
  ...
  description              = "Allow ALB to talk to frontend on 80"
}
...
resource "aws_security_group_rule" "backend_ingress_from_alb_8080" {
  ...
  source_security_group_id = aws_security_group.alb_sg.id
  description              = "Allow ALB to talk to backend on 8080"
}
...
resource "aws_lb" "go_demo_alb" {
  ...
  subnets            = [aws_subnet.go_demo_public_subnet_1.id, aws_subnet.go_demo_public_subnet_2.id]
...
resource "aws_lb_listener_rule" "api_path_listener_rule" {
...
resource "aws_iam_role_policy_attachment" "ecs_task_execution_role_attachment" {
  ...
  policy_arn = data.aws_iam_policy.ecs_task_execution_managed.arn
}
