...
output "frontend_ecr_repository_uri" {
  description = "Amazon ECR Repository URI for the Frontend image (without tag)"
  value       = format("%s.dkr.ecr.%s.amazonaws.com/%s", var.account_id, var.region, var.service_repositories["go-demo-frontend-service"])
}

output "backend_ecr_repository_uri" {
  description = "Amazon ECR Repository URI for the Backend image (without tag)"
  value       = format("%s.dkr.ecr.%s.amazonaws.com/%s", var.account_id, var.region, var.service_repositories["go-demo-backend-service"])
}
...
output "deployment_contract" {
  value = {
    ...
    network = {
      ...
      ingress_id         = aws_lb.go_demo_alb.id
    }
