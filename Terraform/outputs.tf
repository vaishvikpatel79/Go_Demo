...
output "frontend_ecr_image_uri" {
  description = "Full ECR image URI (with tag) for the frontend image as constructed from service_tags"
  value       = try(local.service_images["frontend-service"], null)
}

output "backend_ecr_image_uri" {
  description = "Full ECR image URI (with tag) for the backend image as constructed from service_tags"
  value       = try(local.service_images["backend-service"], null)
}

output "frontend_ecr_repository_uri" {
  description = "ECR repository URI (without tag) for the frontend image"
...
output "backend_ecr_repository_uri" {
  description = "ECR repository URI (without tag) for the backend image"
...
output "deployment_contract" {
  description = "Machine-readable deployment contract for orchestration agents"
  value = {
    meta = {
    ...
    compute = {
      ...
      service_name  = null
      service_names = {
        frontend  = aws_ecs_service.frontend_ecs_service.name
        backend   = aws_ecs_service.backend_ecs_service.name
      }
      task_family   = null
      workload_name = null
    }
