...
echo "[compose] Project: go-demo"
$COMPOSE_CMD -p "go-demo" -f "docker-compose.yml" build
$COMPOSE_CMD -p "go-demo" -f "docker-compose.yml" create || true
'''
            }
        }
...
echo "frontend|frontend|go-demo-frontend" >> compose_service_map.txt
echo "backend|api|go-demo-backend" >> compose_service_map.txt
...
'''
        ...
        stage('Verify Local Images') {
            steps {
                script {
                    def targetImages = [
                    [name: 'go-demo-frontend', tag: 'v1'],
                    [name: 'go-demo-backend', tag: 'v1']
                ]
            ...
            steps {
                script {
                    ...
                    if (userRegistry && !userRegistry.contains('localhost')) {
                        ...
                        if (!accountId) {
                            error("AWS Account ID could not be resolved. Please install AWS CLI or pass account_id.")
                        }
                        env.AWS_ACCOUNT_ID = accountId
                        env.AWS_REGION = 'us-east-1'
                        env.TARGET_REGISTRY = "${accountId}.dkr.ecr.us-east-1.amazonaws.com"
                        echo "Resolved Cloud ECR Registry: ${env.TARGET_REGISTRY}"
                    }
                }
            }
        }
        ...
        stage('Ensure Repositories Exist') {
            steps {
                script {
                    ...
                    for (img in targetImages) {
                        def repoName = img.name
                        def checkRepo = sh(
                            script: "aws ecr describe-repositories --repository-names ${repoName} --region ${env.AWS_REGION ?: 'us-east-1'} 2>/dev/null || true",
                            returnStdout: true
                        ).trim()
                        if (!checkRepo) {
                            echo "Creating ECR repository: ${repoName}"
                            sh "aws ecr create-repository --repository-name ${repoName} --region ${env.AWS_REGION ?: 'us-east-1'}"
                        } else {
                            echo "ECR repository ${repoName} exists."
                        }
                    }
                }
            }
        ...
        stage('Login To Registry') {
            steps {
                script {
                    sh "aws ecr get-login-password --region ${env.AWS_REGION ?: 'us-east-1'} | docker login --username AWS --password-stdin ${env.TARGET_REGISTRY}"
                }
            }
        }
            ...
            steps {
                script {
                    ...
                    for (img in targetImages) {
                        ...
                        pushResults.add('{"image_name": "' + img.name + '", "image_tag": "' + tagToUse + '", "image_uri": "' + remoteUri + '", "push_status": "success"}')
                    }
